...
//					updateView();
		...
		mIcaoWarningMap = new HashMap<NIcaoWarnings, String>();
		mIcaoWarningMap.put(NIcaoWarnings.FACE_NOT_DETECTED, "Face not detected");
		mIcaoWarningMap.put(NIcaoWarnings.EXPRESSION, "Expression");
		mIcaoWarningMap.put(NIcaoWarnings.DARK_GLASSES, "Dark glasses");
		mIcaoWarningMap.put(NIcaoWarnings.BLINK, "Blink");
		mIcaoWarningMap.put(NIcaoWarnings.MOUTH_OPEN, "Mouth open");
		mIcaoWarningMap.put(NIcaoWarnings.LOOKING_AWAY, "Looking away");
		mIcaoWarningMap.put(NIcaoWarnings.RED_EYE, "Red eye");
		mIcaoWarningMap.put(NIcaoWarnings.FACE_DARKNESS, "Face darkness");
		mIcaoWarningMap.put(NIcaoWarnings.UNNATURAL_SKIN_TONE, "Unnatural skin tone");
		mIcaoWarningMap.put(NIcaoWarnings.WASHED_OUT, "Colors washed out");
		mIcaoWarningMap.put(NIcaoWarnings.PIXELATION, "Pixelation");
		mIcaoWarningMap.put(NIcaoWarnings.SKIN_REFLECTION, "Skin reflection");
		mIcaoWarningMap.put(NIcaoWarnings.GLASSES_REFLECTION, "Glasses reflection");
		mIcaoWarningMap.put(NIcaoWarnings.ROLL_LEFT, "Roll left");
		mIcaoWarningMap.put(NIcaoWarnings.ROLL_RIGHT, "Roll right");
		mIcaoWarningMap.put(NIcaoWarnings.YAW_LEFT, "Yaw left");
		mIcaoWarningMap.put(NIcaoWarnings.YAW_RIGHT, "Yaw right");
		mIcaoWarningMap.put(NIcaoWarnings.PITCH_UP, "Pitch up");
		mIcaoWarningMap.put(NIcaoWarnings.PITCH_DOWN, "Pitch down");
		mIcaoWarningMap.put(NIcaoWarnings.TOO_NEAR, "Too near");
		mIcaoWarningMap.put(NIcaoWarnings.TOO_FAR, "Too far");
		mIcaoWarningMap.put(NIcaoWarnings.TOO_NORTH, "Too north");
		mIcaoWarningMap.put(NIcaoWarnings.TOO_SOUTH, "Too south");
		mIcaoWarningMap.put(NIcaoWarnings.TOO_WEST, "Too west");
		mIcaoWarningMap.put(NIcaoWarnings.TOO_EAST, "Too east");
		mIcaoWarningMap.put(NIcaoWarnings.SHARPNESS, "Sharpness");
		mIcaoWarningMap.put(NIcaoWarnings.GRAYSCALE_DENSITY, "Grayscale density");
		mIcaoWarningMap.put(NIcaoWarnings.SATURATION, "Saturation");
		mIcaoWarningMap.put(NIcaoWarnings.BACKGROUND_UNIFORMITY, "Background uniformity");
	...
	private synchronized void updateView() {
		if (DEBUG) Log.i(TAG, "Updating view");
		updateImage();
		updateAttributes();
	}

	private void updateAttributes() {
		if (DEBUG) Log.i(TAG, "Updating attributes");
	...
	private synchronized void updateImage() {
		...
		if (DEBUG) Log.i(TAG, "mImageView Width: " + mImageView.getWidth() + "Image Height: " + mImageView.getHeight());
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		if (DEBUG) Log.i(TAG, "layoutSurface");
		int rotation = 0;
					...
					synchronized(lockObject) {
						try {
							...
							if (mImageToRender != null) {
								...
								if (DEBUG) Log.i(TAG, "ImageRenderThread NFaceVerificationView drawImage, surface: " + mSurface);
	...
	private class NFaceAttributesView extends TextureView implements SurfaceTextureListener {
		...
		private class AttributeRenderThread extends Thread {
			...
			@Override
			public void run() {
				if (DEBUG) Log.i(TAG, "AttributeRenderThread runing task");
				while (mAttributesTextureViewAvailable) {
					if (DEBUG) Log.i(TAG, "AttributeRenderThread mAttributesWaitingLock.block");
					mAttributesWaitingLock.block();
					mAttributesWaitingLock.close();
					if (DEBUG) Log.i(TAG, "AttributeRenderThread mAttributesWaitingLock continue");
					try {
						...
						if (DEBUG) Log.i(TAG, "AttributeRenderThread lockCanvas()");
						Canvas canvas = lockCanvas();
						if (DEBUG) Log.i(TAG, "Canvas: " + canvas);
						if (canvas != null) {
							if (DEBUG) Log.i(TAG, "AttributeRenderThread clearing color");
							canvas.drawColor(Color.TRANSPARENT, Mode.CLEAR);
							if (mEventPropertiesToRender != null) {
								...
								if (DEBUG) Log.i(TAG, "AttributeRenderThread drawLivenessComponent");
							...
							if (DEBUG) Log.i(TAG, "AttributeRenderThread unlockCanvasAndPost");
							unlockCanvasAndPost(canvas);
							if (DEBUG) Log.i(TAG, "AttributeRenderThread unlockCanvasAndPost sucess");
						}
						mTextureViewAvailabilityLock.release();
					} catch (InterruptedException e) {
		...
		private void internalPostImageAndEvent(EventPropertiesHolder eventProperties) {
			...
			if (DEBUG) Log.i(TAG, "internalPostImageAndEvent mAttributesWaitingLock.open");
			mAttributesWaitingLock.open();
		}

		private void startRendering(SurfaceTexture surface) {
			if (DEBUG) Log.i(TAG, "AttributeRenderThread startRendering");
			try {
				mTextureViewAvailabilityLock.acquire();
				mAttributesTextureViewAvailable = true;
				mTextureViewAvailabilityLock.release();
				new AttributeRenderThread().start();
				if (DEBUG) Log.i(TAG, "AttributeRenderThread mAttributesWaitingLock.open()");
				mAttributesWaitingLock.open();
			} catch (InterruptedException e) {
				e.printStackTrace();
			}
		...
		private void stopRendering(SurfaceTexture surface) {
			if (DEBUG) Log.i(TAG, "AttributeRenderThread stopRendering");
			try {
				mTextureViewAvailabilityLock.acquire();
		...
		private void drawLivenessComponent(Canvas canvas, EventPropertiesHolder properties) {
			if (!properties.getLivenessAction().isEmpty()) {
				...
				Rect area = new Rect(x, getHeight() - height * 2, x + width, getHeight() - height);

				if (keepStill) {
					if (score <= 100) {
						actionKey = KEY_LIVENESS_TEXT_KEEP_STILL_WITH_SCORE;
					} else {
					...
					if (score <= 100) {
						actionKey = KEY_LIVENESS_TEXT_KEEP_ROTATING_WITH_SCORE;
					} else {
				...
				canvas.drawText(status.toString(), area.centerX() - (bounds.width() / 2), area.top - dipToPx(DEFAULT_LIVENESS_STATUS_SPACE), mLivenessTextPaint);
				canvas.restoreToCount(oldCanvas);
			}
		}

		private void drawText(Canvas canvas, Rect area, String text) {
			Rect textBounds = new Rect();
			mLivenessTextPaint.getTextBounds(text.toString(), 0, text.length(), textBounds);
			canvas.drawText(text.toString(), area.centerX() - (textBounds.width() / 2), area.top - dipToPx(DEFAULT_LIVENESS_STATUS_SPACE + 15), mLivenessTextPaint);
		...
		@Override
		public void onSurfaceTextureSizeChanged(SurfaceTexture surface, int width, int height) {
			if (DEBUG) Log.i(TAG, "onSurfaceTextureSizeChanged");
		}

		@Override
		public void onSurfaceTextureUpdated(SurfaceTexture surface) {
			if (DEBUG) Log.i(TAG, "onSurfaceTextureUpdated");
		}

	}

	private class EventPropertiesHolder {
